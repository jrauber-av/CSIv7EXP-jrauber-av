...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// iCON V1M Display Processor With Color Auto Update Functionality
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

// Forward declaration - add this before TrackColorMonitor class
class iCON_V1MDisplay_Midi_FeedbackProcessor;
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Color change monitoring system
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

class TrackColorMonitor {
private:
    static TrackColorMonitor* instance_;
    int lastProjState_;
    std::map<MediaTrack*, int> trackColorCache_;
    std::vector<iCON_V1MDisplay_Midi_FeedbackProcessor*> registeredProcessors_;
...
public:
    static TrackColorMonitor* GetInstance() {
        if (!instance_) {
            instance_ = new TrackColorMonitor();
        }
        return instance_;
    }

    void RegisterProcessor(iCON_V1MDisplay_Midi_FeedbackProcessor* processor) {
        registeredProcessors_.push_back(processor);
    }

    void UnregisterProcessor(iCON_V1MDisplay_Midi_FeedbackProcessor* processor) {
        auto it = std::find(registeredProcessors_.begin(), registeredProcessors_.end(), processor);
        if (it != registeredProcessors_.end()) {
            registeredProcessors_.erase(it);
        }
    }

    void CheckProjectState() {
        int projState = GetProjectStateChangeCount(0);

        if (projState != lastProjState_) {
            lastProjState_ = projState;
            OnProjectStateChange(projState);
        }
    }

    void OnProjectStateChange(int state) {
        // Check colors on any state change
        CheckAllTrackColors();
    }
    ...
    void CheckAllTrackColors() {
        ...
        for (int i = 0; i < trackCount; i++) {
            ...
            if (track) {
                int currentColor = GetTrackColor(track);

                auto it = trackColorCache_.find(track);
                if (it == trackColorCache_.end() || it->second != currentColor) {
                    trackColorCache_[track] = currentColor;
                    hasChanges = true;
                }
            }
        }
...
inline TrackColorMonitor* TrackColorMonitor::instance_ = nullptr;

// Timer callback with debug
inline void ColorMonitorTimerProc() {
    
    TrackColorMonitor::GetInstance()->CheckProjectState();
}
...
private:
    ...
    double lastColorUpdateTime_;
    static const unsigned int COLOR_UPDATE_THROTTLE_MS = 50;
    bool pendingColorUpdate_;
    double pendingUpdateTime_;

public:
    virtual ~iCON_V1MDisplay_Midi_FeedbackProcessor() {
        if (colorMonitoringEnabled_) {
            TrackColorMonitor::GetInstance()->UnregisterProcessor(this);
        }
    }

    iCON_V1MDisplay_Midi_FeedbackProcessor(CSurfIntegrator* const csi, Midi_ControlSurface* surface, Widget* widget, int displayUpperLower, int displayType, int displayRow, int channel)
        ...
        lastColorUpdateTime_(0),
        pendingColorUpdate_(false),
        pendingUpdateTime_(0)
    {
        preventUpdateTrackColors_ = false;
        ...
        TrackColorMonitor::GetInstance()->RegisterProcessor(this);
    ...
    virtual void SetV1MDisplayColors(const char* colors) override
    {
    ...
    void OnTrackColorsChanged() {
        if (!colorMonitoringEnabled_ || preventUpdateTrackColors_)
            return;

        double currentTime = time_precise() * 1000.0;

        pendingColorUpdate_ = true;
        pendingUpdateTime_ = currentTime + COLOR_UPDATE_THROTTLE_MS;

        if ((currentTime - lastColorUpdateTime_) >= COLOR_UPDATE_THROTTLE_MS) {
            ProcessColorUpdate();
        }
    }

    void CheckPendingUpdate() {
        if (pendingColorUpdate_) {
            double currentTime = time_precise() * 1000.0;
            if (currentTime >= pendingUpdateTime_) {
                ProcessColorUpdate();
            }
        }
    }
    ...
    void ProcessColorUpdate() {
        pendingColorUpdate_ = false;
        lastColorUpdateTime_ = time_precise() * 1000.0;

        bool hasChanges = false;
        for (int i = 0; i < surface_->GetNumChannels(); ++i) {
            rgba_color newColor = surface_->GetTrackColorForChannel(i);
            if (i < currentTrackColors_.size()) {
    ...
    void SetColorMonitoringEnabled(bool enabled) {
        colorMonitoringEnabled_ = enabled;
    }
...
// NOW implement the TrackColorMonitor methods
inline void TrackColorMonitor::NotifyProcessors() {
    for (auto processor : registeredProcessors_) {
        if (processor) {
            processor->OnTrackColorsChanged();
        }
    }
}

inline void TrackColorMonitor::ForceRefresh() {
    NotifyProcessors();
}

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// iCON V1M Global functions for manual control
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

inline void ForceRefreshV1MDisplays() {
    TrackColorMonitor::GetInstance()->ForceRefresh();
}

inline void ShutdownV1MColorMonitoring() {
    plugin_register("-timer", (void*)ColorMonitorTimerProc);
}
